...
# Volume (now based on PALM SCALE, not bbox area)
...
SPREAD_START = 0.55         # below this -> no vibrato
SPREAD_FULL = 0.95          # at/above this -> max vibrato
...
def finger_horizontal_spread_norm(hand_landmarks, palm_scale: float) -> float:
    """
    Your definition: "how close together the fingers are horizontally"
    Use PIP joints x-range (6,10,14,18) and normalize by palm scale (0-9)
    so distance-to-camera doesn't accidentally trigger vibrato.
    """
    pip_idxs = [6, 10, 14, 18]
...
def main() -> None:
    ...
    # Volume calibration now uses palm scale (not bbox area)
    ...
    with HandLandmarker.create_from_options(options) as landmarker:
        while True:
            ...
            if hand_present:
                ...
                # Spread uses PIP x-range normalized by palm scale (not bbox width)
                ...
                # Default palm calibration bootstrap (so it works immediately)
                if palm_min is None or palm_max is None:
                    palm_min = float(ema_palm) * 0.85
                    palm_max = float(ema_palm) * 1.25

                # Ensure y_top/y_bottom ordering is correct
                if y_bottom < y_top:
                    y_top, y_bottom = y_bottom, y_top

                # Linear zone mapping within calibrated range (equal slices)
                ...
                # Dwell/hysteresis for zone switching
                ...
                # Volume from palm scale ONLY (distance proxy, decoupled from finger spread)
                ...
                # Chord switching with voice-leading: keep common notes held
                ...
                # Vibrato depth from horizontal finger spread ONLY (now independent of distance)
                ...
                last_result_data = {
                    ...
                    "finger_spread_x_norm": float(spread),
                ...
                if SHOW_DEBUG_TEXT:
                    ...
                    cv2.putText(frame_bgr, f"Expr:{last_expr_sent}  PalmScale:{ema_palm:.4f}  SpreadX:{ema_spread:.3f}  Vib:{depth}",
                                (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)

            else:
                # No hand: hold chord briefly, otherwise stop + reset pitchwheel
                if not grace_ok:
                    if active_notes:
            ...
            # Volume calibration now uses palm scale
